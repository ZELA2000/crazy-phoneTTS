...
# Default: accept connections from all IPs
BACKEND_HOST = os.getenv("BACKEND_HOST", "0.0.0.0")
BACKEND_PORT = int(os.getenv("BACKEND_PORT", "8000"))  # Default: port 8000

if not AZURE_SPEECH_KEY:
    logger.error(
        "AZURE_SPEECH_KEY non configurata! Configura la variabile d'ambiente AZURE_SPEECH_KEY.")
    logger.info(
        "Per ottenere una chiave Azure Speech: https://docs.microsoft.com/azure/cognitive-services/speech-service/get-started")
else:
    logger.info("Azure Speech Services configurato correttamente")

logger.info(
    f"Server configurato per ascoltare su: {BACKEND_HOST}:{BACKEND_PORT}")
    ...
    return [
        {
            ...
            "voice": row[2],
            "timestamp": row[3],
            # Solo ultimi 8 caratteri IP per privacy
            "user_ip": row[4][-8:] if row[4] else "unknown"
        }
        for row in rows
    ]
...
def convert_audio_to_format(audio_segment, output_format, audio_quality, custom_filename):
    ...
    if output_format == "wav":
        ...
        audio_segment.export(output_path, format="mp3",
                             bitrate="128k" if audio_quality == "pcm" else "64k")
...
async def test_azure_speech_connection():
    ...
    try:
        ...
        logger.debug(
            f"Key length: {len(AZURE_SPEECH_KEY) if AZURE_SPEECH_KEY else 0}")

        # Configurazione test Azure Speech
        if AZURE_SPEECH_ENDPOINT:
            speech_config = speechsdk.SpeechConfig(
                subscription=AZURE_SPEECH_KEY, endpoint=AZURE_SPEECH_ENDPOINT)
            logger.debug(
                f"Using custom endpoint for test: {AZURE_SPEECH_ENDPOINT}")
        else:
            speech_config = speechsdk.SpeechConfig(
                subscription=AZURE_SPEECH_KEY, region=AZURE_SPEECH_REGION)
            logger.debug(
                f"Using standard region for test: {AZURE_SPEECH_REGION}")
        ...
        try:
            ...
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                logger.info(
                    f"✅ Azure Speech test successful - Generated {len(result.audio_data)} bytes")
                return True
            ...
            try:
                ...
                synthesizer_fallback = speechsdk.SpeechSynthesizer(
                    speech_config=speech_config, audio_config=None)
                ...
                if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                    logger.info(
                        f"✅ Azure Speech fallback test successful with standard voice")
                    logger.warning(
                        "⚠️ Neural voices may be slow in your network environment")
                    return True
        ...
        if result.reason == speechsdk.ResultReason.Canceled:
            ...
            if "timeout" in error_details['error_details'].lower() or "SynthesisConnectionTimeoutMs" in str(timeout_error):
                ...
                logger.error(
                    "   - Assicurati che la risorsa Speech Services sia attiva")
            elif "region" in error_details['error_details'].lower():
                logger.error(
                    f"💡 PROBLEMA: Region '{AZURE_SPEECH_REGION}' non disponibile")
                logger.error("   - Verifica la region nel portale Azure")
                logger.error(
                    "   - Prova region alternative: eastus, westus2, northeurope")

            raise Exception(
                f"Azure Speech test failed: {error_details['reason']} - {error_details['error_details']}")
        else:
            raise Exception(
                f"Azure Speech test failed with reason: {result.reason}")
    ...
    try:
        ...
        if AZURE_SPEECH_ENDPOINT:
            speech_config = speechsdk.SpeechConfig(
                subscription=AZURE_SPEECH_KEY, endpoint=AZURE_SPEECH_ENDPOINT)
            logger.info(
                f"Using endpoint configuration: {AZURE_SPEECH_ENDPOINT}")
        else:
            speech_config = speechsdk.SpeechConfig(
                subscription=AZURE_SPEECH_KEY, region=AZURE_SPEECH_REGION)
        ...
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            logger.info(
                f"✅ Speech synthesized successfully - {len(result.audio_data)} bytes")
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                logger.info(
                    f"✅ Azure Speech synthesis completed: {os.path.getsize(output_path)} bytes")
                return True
            else:
...
def generate_ssml(text: str, voice: str, options: dict) -> str:
    ...
    # x-slow, slow, medium, fast, x-fast o percentuale
    rate = options.get('rate', 'medium')
    # x-low, low, medium, high, x-high o frequenza
    ...
    # cheerful, sad, excited, etc. (per voci Neural)
    style = options.get('style', None)
    # Intensità dello stile (0.01-2.0)
    style_degree = options.get('style_degree', '1.0')
    ...
    if style and 'Neural' in voice:
        ...
        ssml_parts.append(
            f'<mstts:express-as style="{style}" styledegree="{style_degree}" xmlns:mstts="https://www.w3.org/2001/mstts">')
    else:
        ssml_parts.append(f'<voice name="{voice}">')
...
async def upload_music_to_library(
    ...
    if music_file.content_type not in audio_types:
        raise HTTPException(
            status_code=400, detail="File deve essere audio (MP3, WAV, OGG)")
...
async def delete_music_from_library(song_id: str):
    ...
    if not os.path.exists(metadata_path):
        raise HTTPException(
            status_code=404, detail="Canzone non trovata nella libreria")
    ...
    try:
        ...
        logger.info(
            f"Music deleted from library: {metadata['name']} ({song_id})")
...
def get_current_version():
    ...
    version_paths = [
        ...
        # Directory padre (se backend è in sottocartella)
        "../VERSION",
        "/app/VERSION",              # Container Docker
        "./VERSION",                 # Esplicito directory corrente
        os.path.join(os.path.dirname(__file__), "..",
                     "VERSION")  # Relativo a questo file
    ]
...
async def check_updates():
    ...
    return {
        ...
        "update_instructions": update_instructions if update_available else None
    }
...
async def start_update():
    ...
    try:
        ...
        if not update_info["update_available"]:
            ...
            raise HTTPException(
                status_code=400, detail="Nessun aggiornamento disponibile")
        ...
        run_update_script_background(
            script_path, update_info["latest_version"])
...
async def trigger_blue_green_deploy():
    """Triggera il Blue-Green deployment eseguendo lo script Python"""
    try:
        # Salva progress iniziale
        save_update_progress({
            "type": "progress",
            "step": "deploy_start",
            "message": "🚀 Avvio Blue-Green deployment...",
            "percentage": 5,
            "status": "running"
        })
        ...
        if not update_info["update_available"]:
            save_update_progress({
                "type": "info",
                "step": "no_update",
                "message": "ℹ️ Nessun aggiornamento disponibile",
                "percentage": 100,
                "status": "completed"
            })
            return {
        ...
        # Esegui deploy in background
        import subprocess
        deploy_script = "/app/project/scripts/blue_green_deploy.py"

        # Nota: Questo deve essere eseguito dall'HOST, non dal container
        # Per ora creiamo un file di richiesta deploy
        deploy_request = {
            "version": update_info["latest_version"],
            "timestamp": datetime.now().isoformat(),
            "status": "requested",
        ...
        deploy_request_file = "/app/project/deploy_request.json"
        with open(deploy_request_file, 'w') as f:
            json.dump(deploy_request, f, indent=2)

        save_update_progress({
            "type": "progress",
            "step": "waiting_host",
            "message": "⏳ Richiesta deploy creata. Esegui manualmente: py scripts/blue_green_deploy.py deploy",
            "percentage": 10,
            "status": "waiting_manual",
            "info": "Il Blue-Green deployment deve essere eseguito dall'host. Usa: py scripts/blue_green_deploy.py deploy"
        })

        return {
            "message": "Richiesta deploy creata. Esegui manualmente il deploy dall'host.",
            "version": update_info["latest_version"],
            "status": "manual_required",
            "command": "py scripts/blue_green_deploy.py deploy"
        }

    except Exception as e:
        logger.error(f"Errore trigger deploy: {e}")
        save_update_progress({
            "type": "error",
            "step": "error",
            "message": f"❌ Errore: {str(e)}",
